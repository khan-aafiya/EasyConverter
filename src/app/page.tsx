...
import { UploadCloud, FileDown, GripVertical, X, Loader2 } from 'lucide-react';
...
export default function Home() {
  ...
  const onDragEnter = (e: DragEvent<HTMLDivElement>, index: number) => {
    ...
    const allImages = JSON.parse(JSON.stringify(newImages));
    const draggedItem = allImages.splice(dragImage.current, 1)[0];
    allImages.splice(dragOverImage.current, 0, draggedItem);
    
    // As we can't store File object in JSON, we need to re-map it.
    const reorderedImages = allImages.map((item: {id: string}) => images.find(img => img.id === item.id)!);
